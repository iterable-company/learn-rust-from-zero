...
use nix::{
    ...
    unistd::{
        self, close, dup2, execvp, fork, pipe, setpgid, tcgetpgrp, tcsetpgrp, ForkResult, Pid,
    },
};
...
impl Worker {
                ...
                match msg {
                    WorkerMsg::Cmd(line) => {
                        match parse_cmd(&line) {
                            Ok((cmd, redirect_filename)) => {
                                if self.built_in_cmd(&cmd, &shell_tx) {
                                    // 組み込みコマンドならworker_rxから受信
                                    continue;
                                }

                                if !self.spawn_child(&line, &cmd, redirect_filename) {
    ...
    fn spawn_child(
        ...
        line: &str,
        cmd: &[(&str, Vec<&str>)],
        redirect_filename_opt: Option<&str>,
    ) -> bool {
        ...
        let redirect = if let Some(redirect_filename) = redirect_filename_opt {
            let control_removed = redirect_filename
                ...
                .collect::<String>();
            let path = Path::new(&control_removed);
            Some(
                match open(
                    path,
                    ...
                    Ok(fd) => fd,
                    Err(e) => {
                        eprintln!("ZeroSh: ファイルが開けませんでした。: {redirect_filename}");
                        eprintln!("ZeroSh: ファイルが開けませんでした。: {e}");
                        return false;
                    }
                },
            )
...
/// > でsplit
fn parse_redirect(line: &str) -> Result<Vec<&str>, DynError> {
    let split: Vec<&str> = line.split('>').collect();
    if split.len() > 2 {
        return Err("> は最後に一度指定できるだけです".into());
    }
    Ok(split)
}

type CmdResult<'a> = Result<(Vec<(&'a str, Vec<&'a str>)>, Option<&'a str>), DynError>;
